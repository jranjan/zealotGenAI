...
class AssetInsightDashboard:
    ...
    def render_sidebar(self):
            ...
            if st.button("🔄 Run Analysis", type="primary"):
                with st.spinner("Running asset analysis..."):
                    if self.load_analysis_data(final_config_path):
                        ...
                        st.sidebar.error("❌ Failed to run analysis")
        ...
        # Cloud selection
        st.sidebar.subheader("☁️ Cloud Filter")
    ...
    def cleanup_temp_files(self):
        ...
        for temp_file in self.temp_files:
            try:
                ...
                st.warning(f"⚠️ Could not remove temporary file {temp_file}: {e}")
        self.temp_files = []
    ...
    def render_summary_metrics(self):
        ...
        st.markdown('<div class="main-header">📊 Asset Insight Dashboard</div>', unsafe_allow_html=True)
    ...
    def render_cloud_details(self, selected_clouds, selected_teams):
        ...
        for cloud_name in selected_clouds:
            ...
            with st.expander(f"☁️ {cloud_name} ({cloud_info.get('total_cloud_assets', 0)} assets, {cloud_info.get('total_cloud_team', 0)} teams)", expanded=False):
                ...
                if teams:
                    st.subheader(f"👥 Teams in {cloud_name}")
                    ...
                    for i, team in enumerate(teams):
                        ...
                        with st.expander(f"👥 {team_name} ({len(team_assets)} assets)", expanded=False):
                            ...
                            if team_assets:
                                ...
                                search_term = st.text_input(f"🔍 Search assets in {team_name}", key=f"search_{cloud_name}_{i}")
                                if search_term:
    ...
    def run(self):
        ...
        if not self.analysis_data:
            st.info("👈 Please load analysis data using the sidebar to get started.")
            st.markdown("""
            **To get started:**
        ...
        tab1, tab2, tab3, tab4 = st.tabs(["📊 Overview", "📋 Details", "📈 Analytics", "🔍 Analysis Results"])
